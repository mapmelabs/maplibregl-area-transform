...
export class MaplibreAreaTransform implements IControl {
  ...
  public onAdd(map: Map): HTMLElement {
    ...
    this._container.className =
      "maplibregl-ctrl maplibregl-ctrl-area-transform";
   ...
   */
  public createCoordinatesForLoadedImage(
  ...
  public addImage(
    optionsOrUrl: AddImageOptions | string,
    coordinates?: GeoJSON.Position[],
  ): Promise<string> {
    const options: AddImageOptions =
      typeof optionsOrUrl === "string"
        ? { imageUrl: optionsOrUrl, coordinates: coordinates! }
        : optionsOrUrl;
  ...
  private async addImageInternal(options: AddImageOptions): Promise<string> {
    ...
    const image = (await this._map!.loadImage(options.imageUrl)).data;
    const imageWidth =
      "naturalWidth" in image ? image.naturalWidth : image.width;
    const imageHeight =
      "naturalHeight" in image ? image.naturalHeight : image.height;
    const canvasScale = Math.min(1, 2048 / imageWidth, 2048 / imageHeight);
    const canvas = document.createElement("canvas");
    canvas.width = Math.max(1, Math.round(imageWidth * canvasScale));
    canvas.height = Math.max(1, Math.round(imageHeight * canvasScale));
    this._imageCanvases.set(imageId, { image, canvas });
    this.paintImageCanvas(imageId, options.coordinates);
    ...
    await geojsonSource.updateData(
      {
        add: this.buildPolygonGeoJSONFeatures({
  ...
  private syncImageCoordinates(
    ...
    const source = this._map?.getSource<CanvasSource>(
      `${IMAGE_SOURCE_PREFIX}${featureId}`,
    );
    if (!source || !this._imageCanvases.has(featureId)) {
      this._map
        ?.getSource<ImageSource>(`${IMAGE_SOURCE_PREFIX}${featureId}`)
        ?.setCoordinates(coordinates as Coordinates);
      return;
    }
  ...
  public async setImageOpacity(
    imageId: string,
    opacity: number,
  ): Promise<void> {
    this._map?.setPaintProperty(
      `${IMAGE_LAYER_PREFIX}${imageId}`,
      "raster-opacity",
      opacity,
    );
  }
  ...
  public addRectangle(): Promise<string> {
    ...
    const startY =
      (logicalHeight * (1 - this.options.rectangleSizeFactor!)) / 2;
  ...
  public async deleteFeature(featureId: string) {
    ...
    const imageSource = this._map?.getSource<ImageSource>(
      IMAGE_SOURCE_PREFIX + featureId,
    );
    if (imageSource) {
    ...
    this._imageCanvases.delete(featureId);
  ...
  public async isSelectedFeatureRectangle(): Promise<boolean> {
    if (!this._selectedFeatureId) return false;
    const data = (await this._map
      ?.getSource<GeoJSONSource>(GEOJSON_SOURCE)
      ?.getData()) as GeoJSON.FeatureCollection | undefined;
    if (!data) return false;
    const corners = data.features.filter(
      (f): f is GeoJSON.Feature<GeoJSON.Point> =>
        f.geometry.type === "Point" &&
        f.properties?.["featureId"] === this._selectedFeatureId &&
        f.properties?.["type"] === "scale-handle",
    );
    if (corners.length !== 4) return false;
    return pxIsRectangle(
      this.projectAll(corners.map((f) => f.geometry.coordinates)),
    );
  }
  ...
  public async resetSelectedFeaturePlacement(imageUrl: string): Promise<void> {
    ...
    this.syncImageCoordinates(this._selectedFeatureId, coordinates);
    await this.updateCoordinates(this._selectedFeatureId, coordinates);
  }
  ...
  private buildPolygonGeoJSONFeatures(
    buildOptions: BuildPolygonOptions,
  ): GeoJSON.Feature[] {
  ...
  private getRotateHandlePoint(
    ...
    const handlePx: PxPoint = [
      midPx[0] + normalPx[0] * offsetDist,
      midPx[1] + normalPx[1] * offsetDist,
    ];
  ...
  private getScaleHandleHeadingSnapped(
    ...
    const direction =
      (((centerToFirstSecondAngle + centerToFirstPointAngle) / 2) *
        (180 / Math.PI) +
        180) %
      180;
  ...
  private onMouseMoveForCursor = (e: MapMouseEvent) => {
    ...
    const features = this._map
      ...
      .filter((f) => f.properties?.["featureId"] === this._selectedFeatureId);
    const rotate = features?.find(
      (f) =>
        f.layer.id.startsWith(HANDLE_LAYER) &&
        f.properties["type"] === "rotate-handle",
    );
  ...
  private async setStateFromMouseDown(
    currentPx: PxPoint,
    queriedFeatures: MapGeoJSONFeature[],
  ) {
    const data = (await this._map
      ?.getSource<GeoJSONSource>(GEOJSON_SOURCE)
    ...
    for (const feature of featurePoints) {
      ...
      const bestPx = this.project(
        (closestFeature.geometry as GeoJSON.Point).coordinates,
      );
    ...
    this._startPx = this.project(
      (closestFeature.geometry as GeoJSON.Point).coordinates,
    );
    if (
      closestFeature.properties?.["type"] === "scale-handle" &&
      this.options.quadrilateralMode
    ) {
      ...
      if (this._dragCornerIdx < 0) {
        this._dragCornerIdx = this._startCornersPx.reduce(
          (best, c, i, arr) =>
            pxDistance(c, this._startPx!) <
            pxDistance(arr[best]!, this._startPx!)
              ? i
              : best,
          0,
        );
      ...
      closestFeature.properties?.["type"] === "scale-handle" &&
      this._selectedFeatureId?.startsWith(RESIZEABLE_POLYGON_FEATURE_ID)
    ) {
  ...
  private onMouseMove = (e: MapMouseEvent) => {
    ...
    switch (this._state) {
      case "rotating":
        newCornersPx = pxRotatePolygon(
          this._startCornersPx!,
          this._startPx,
          currentPx,
        );
        break;
      case "scaling":
        newCornersPx = pxScalePolygon(
          this._startCornersPx!,
          this._startPx,
          currentPx,
        );
    ...
    this.syncImageCoordinates(this._selectedFeatureId, newCoordinates);
  ...
  private onClick = (e: MapMouseEvent) => {
    ...
    const polygonFeature = features?.find((f) =>
      f.layer.id.startsWith(AREA_LAYER),
    );
    if (polygonFeature && this._state === "deleting") {
      this.deleteFeature(polygonFeature.properties["featureId"]);
      return;
    }
    const targetId: string | null = polygonFeature
      ? polygonFeature.properties["featureId"]
      : null;
  ...
  private async onClickWhenInPolygonMode(e: MapMouseEvent) {
    ...
    const areaGeometry: GeoJSON.Polygon = {
      ...
      coordinates: [
        [
          ...this.unprojectAll(this._polygonPoints),
          this.unproject(this._polygonPoints[0]!),
        ],
      ],
    };
    if (this._polygonPoints.length === 3) {
  ...
  private async updateCoordinates(
    featureId: string,
    newCoordinates: GeoJSON.Position[],
  ) {
    const source = this._map?.getSource<GeoJSONSource>(GEOJSON_SOURCE)!;
    const data = (await source.getData()) as GeoJSON.FeatureCollection;
    const color = data.features.find(
    ...
    data.features.push(
      this.getRotateHandlePoint(newCoordinates, featureId, color),
    );
    await source.setData(data, true);
  ...
  private setState(state: MaplibreAreaTransformState) {
    this._state = state;
    document
      .getElementById(POLYGON_BUTTON_ID)
      ?.classList.toggle("active", this._state === "adding-ploygon");
    document
      .getElementById(DELETE_BUTTON_ID)
      ?.classList.toggle("active", this._state === "deleting");
  }
}
