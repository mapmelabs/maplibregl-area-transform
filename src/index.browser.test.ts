...
function rasterLayers(map: maplibregl.Map) {
  return map
    .getStyle()
    .layers.filter((l) => l.id.startsWith(IMAGE_LAYER_PREFIX));
}
...
async function drawPolygon(
  ...
  const created = new Promise<string>((resolve) =>
    control.on("create", (e) => resolve(e.id)),
  );
...
async function setup(
  options?: MaplibreAreaTransformOptions,
  ...
  it("selects on open, deselects on outside click, reselects on image click", async () => {
    ...
    await waitUntil(() =>
      map
        ...
        .some((f: maplibregl.MapGeoJSONFeature) =>
          f.layer.id.startsWith("area-transform-layer-polygon-area"),
        ),
    );
...
describe("MaplibreAreaTransform image URL queue", () => {
  ...
  it("shares one promise between concurrent calls with the same URL and coordinates", async () => {
    ...
    const secondPromise = control.addImage({
      imageUrl: rotateUrl,
      coordinates,
    });
...
describe("MaplibreAreaTransform rotation", () => {
  ...
  it("rotates the feature when dragging the rotate handle", async () => {
    ...
    const points = data.features.filter(
      (f) =>
        f.geometry.type === "Point" && f.properties?.["featureId"] === rectId,
    ) as GeoJSON.Feature<GeoJSON.Point>[];
    const cornerCoords = points
      .filter((f) => f.properties?.["type"] === "scale-handle")
      .map((f) => f.geometry.coordinates);
    const rotateCoord = points.find(
    ...
    expect(
      Math.hypot(
        movedPx[0] - origCornersPx[0]![0],
        movedPx[1] - origCornersPx[0]![1],
      ),
    ).toBeGreaterThan(5);
  });
});
